...
if [ "$versionPython" -lt 12 ]; then 
	...
	log "**************************************************"
	# PYENV_ROOT="${PYENV_DIR}" ${PYENV_DIR}/bin/pyenv install -s 3.12 | log
...
if [ "$versionPython" -ge 12 ]; then
	...
	if [ "$minorPythonVenv" -ge 12 ]; then
		...
		if [ "$VenvToUpdate" -eq 1 ] || [ "$FORCE_INIT_VENV" -eq 1 ]; then
			...
			# ${PYENV_DIR}/versions/$(PYENV_ROOT="${PYENV_DIR}" ${PYENV_DIR}/bin/pyenv latest 3.12)/bin/python3 -m venv --upgrade-deps ${VENV_DIR} | log
